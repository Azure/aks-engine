# Microsoft Azure Kubernetes Service Engine

The Azure Kubernetes Service Engine (`aks-engine`) generates ARM (Azure Resource Manager) templates for Kubernetes clusters on Microsoft Azure. The input to aks-engine is a cluster definition file which describes the desired cluster, including orchestrator, features, and agents. The structure of the input files is very similar to the public API for Azure Container Service.
...
* Build aks-engine:
  ...
  8. Type `cd %GOPATH%\src\github.com\Azure\aks-engine\pkg\aksengine`
  9. Type `go generate`
