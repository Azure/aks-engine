...
2) `aks-engine upgrade` expects a cluster configuration that conforms to the current state of the cluster. In other words, the Azure resources inside the resource group deployed by `aks-engine` should be in the same state that when they were originally created by `aks-engine`. If you do any manual changes against the resources in the resource group created by `aks-engine` DO NOT use `aks-engine upgrade`, as the aks-engine-generated ARM template won't be reconcilable against the state of the Azure resources that reside in that resource group. This includes:

- Any manual changes to resources via the `az` CLI
- Any manual changes to resources via the Azure Portal
- Any manual changes to resources via an ARM template deployment against the `aks-engine`-created resource group
- Any manual changes to resources via Azure Resource Manager (https://resources.azure.com/)
- Any manual changes via a 3rd party tool (e.g., ansible, terraform, chef) that employ the above resource modification vectors

In summary, the set of Azure resources in the resource group are mutually reconcilable only if they have been exclusively created and managed as the result of a series of successive ARM template deployments originating from `aks-engine`.
