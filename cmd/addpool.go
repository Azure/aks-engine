...
func newAddPoolCmd() *cobra.Command {
	...
	f.StringVarP(&apc.agentPoolPath, "agentpool", "m", "", "path to the generated agentpool.json file")
...
func (sc *addPoolCmd) validate(cmd *cobra.Command) error {
...
func (sc *addPoolCmd) load() error {
	...
	if _, err = os.Stat(sc.agentPoolPath); os.IsNotExist(err) {
	...
	// TODO: Modify this to verify that an agent pool of the same name does not already exist
...
func (sc *addPoolCmd) run(cmd *cobra.Command, args []string) error {
	...
	addValue(parametersJSON, sc.agentPool.Name+"Count", 0)
	...
	if sc.nodes != nil {
		nodes, err := operations.GetNodes(sc.client, sc.logger, sc.apiserverURL, sc.kubeconfig, time.Duration(5)*time.Minute, sc.agentPool.Name, sc.agentPool.Count)
		if err == nil && nodes != nil {
			sc.nodes = nodes
			sc.logger.Infof("Nodes in pool '%s' after scaling:\n", sc.agentPool.Name)
			operations.PrintNodes(sc.nodes)
...
func (sc *addPoolCmd) saveAPIModel() error {
	var err error
	apiloader := &api.Apiloader{
		Translator: &i18n.Translator{
			Locale: sc.locale,
	...
	sc.containerService.Properties.AgentPoolProfiles[sc.agentPoolIndex].Count = sc.agentPool.Count
...
// func (sc *addPoolCmd) vmInAgentPool(vmName string, tags map[string]*string) bool {
...
// func (sc *addPoolCmd) drainNodes(vmsToDelete []string) error {
...
// func (sc *addPoolCmd) printScaleTargetEqualsExisting(currentNodeCount int) {
// 	var printNodes bool
// 	trailingChar := "."
// 	if sc.nodes != nil {
// 		printNodes = true
