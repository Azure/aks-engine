...
make vhd-notes:
	awk '/START_OF_NOTES/{y=1;next}y' packer-output > release-notes-raw.txt && awk '/END_OF_NOTES/ {exit} {print}' release-notes-raw.txt | sed -r "s/\x1B\[([0-9]{1,2}(;[0-9]{1,2})?)?[mGK]//g" | grep -v '^[[:space:]]\{5\}\+' > release-notes.txt && sed -e s/azure-arm://g -i release-notes.txt && cat release-notes.txt