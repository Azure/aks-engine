...
make vhd-notes:
	awk '/cat \/var\/log\/azure\/golden-image-install\.complete/{y=1;next}y' packer-output > release-notes && awk '/rm -f \/root\/\.ssh\/authorized_keys/ {exit} {print}' release-notes > release-notes-trimmed && sed -e s/azure-arm://g -i release-notes-trimmed && cat release-notes-trimmed