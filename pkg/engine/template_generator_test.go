...
func TestGenerateMasterCustomDataSize(t *testing.T) {
	tg, _ := InitializeTemplateGenerator(Context{})

	apiModelStr := get117APIModelString()
	cs := &api.ContainerService{}
	if err := json.Unmarshal([]byte(apiModelStr), &cs); err != nil {
		t.Errorf("unexpected error while unmarshalling the apiModel JSON: %s", err.Error())
	}

	testCloudControllerVersion := "1.17.0-alpha.1"
	tmpl, _, err := tg.GenerateTemplateV2(cs, DefaultGeneratorCode, testCloudControllerVersion)
	if err != nil {
		t.Errorf("unexpected error while running GenerateTemplateV2: %s", err.Error())
	}
	...
	// walk through the ARM template JSON and find "customData" fields
	for _, resource := range resources {
		rsrc := resource.(map[string]interface{})
		if properties, ok := rsrc["properties"]; ok {
			props := properties.(map[string]interface{})
			// osProfile may be under vmMachineProfile or directly under resource properties
			if virtualMachineProfile, ok := props["virtualMachineProfile"]; ok {
				props = virtualMachineProfile.(map[string]interface{})
			}
			if osProfile, ok := props["osProfile"]; ok {
				prof := osProfile.(map[string]interface{})
				if customData, ok := prof["customData"]; ok {
					data := customData.(string)
					customDataLength := len(data)
					if customDataLength > bestGuessAtAcceptableSize {
						t.Errorf("%s\ncustomData is too large at %d bytes", data, customDataLength)
					}
					t.Logf("customData is %d bytes", customDataLength)
				}
			}
		}
	}
