...
func (az *AzureClient) DeployTemplate(ctx context.Context, resourceGroupName, deploymentName string, template map[string]interface{}, parameters map[string]interface{}) (de resources.DeploymentExtended, err error) {
	...
	if err != nil {
		log.Errorf("Finished ARM Deployment (%s). Error: %v", deploymentName, err)
		return de, err
	}

	de, err = future.Result(az.deploymentsClient)
	if err != nil {
		log.Errorf("Finished ARM Deployment (%s). Result Error: %v", deploymentName, err)
		return de, err
	}

	log.Infof("Finished ARM Deployment (%s). Succeeded", deploymentName)
	return de, err
}
