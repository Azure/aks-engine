...
func TestWindowsProfile(t *testing.T) {
	...
	hasLicenseType := w.HasLicenseType()
	if hasLicenseType {
		t.Fatalf("Expected HasLicenseType() to equal default 'false', got %t", hasLicenseType)
	}

	isValidLicenseType := w.IsValidLicenseType()
	if isValidLicenseType {
		t.Fatalf("Expected IsValidLicenseType() to equal default 'false', got %t", isValidLicenseType)
	}

	licenseType := WindowsLicenseTypeServer
	w = WindowsProfile{
	...
	hasLicenseType = w.HasLicenseType()
	if !hasLicenseType {
		t.Fatalf("Expected HasLicenseType() to equal default 'true', got %t", hasLicenseType)
	}

	isValidLicenseType = w.IsValidLicenseType()
	if !isValidLicenseType {
		t.Fatalf("Expected IsValidLicenseType() to equal default 'true', got %t", isValidLicenseType)
	}
