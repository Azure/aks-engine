...
func (cs *ContainerService) setOrchestratorDefaults(isUpdate bool) {
	...
	case Kubernetes:
		...
		if len(cloudSpecConfig.KubernetesSpecConfig.ImageRepoOverrides) > 0 {
			if o.KubernetesConfig.ImageRepoOverrides == nil {
			...
			for k, v := range cloudSpecConfig.KubernetesSpecConfig.ImageRepoOverrides {
				if _, exists := o.KubernetesConfig.ImageRepoOverrides[k]; !exists {
