...
func TestKubernetesContainerSpecHasResourceConfig(t *testing.T) {
	...
	for _, c := range cases {
		if c.c.HasResourceConfig() != c.expected {
			t.Fatalf("expected HasResourceConfig() to return %t but instead returned %t", c.expected, c.c.HasResourceConfig())
		}
	}
}
