...
if ($Global:ClusterConfiguration.Cni.Name -eq "antrea") {
    $global:KubeDir = $Global:ClusterConfiguration.Install.Destination
    $global:KubeproxyArgList = @("--v=3", "--proxy-mode=userspace", "--hostname-override=$env:computername", "--kubeconfig=$KubeDir\config")
