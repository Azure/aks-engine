...
master_nodes() {
    kubectl get no -L kubernetes.azure.com/role -l kubernetes.azure.com/role=master --no-headers -o jsonpath="{.items[*].metadata.name}" | tr " " "\n" | sort | head -n 1
}
...
agent_nodes() {
    kubectl get no -L kubernetes.azure.com/role -l kubernetes.azure.com/role=agent --no-headers -o jsonpath="{.items[*].metadata.name}" | tr " " "\n" | sort | head -n 1
}
...
storageclass_param() {
	if kubectl get no -l kubernetes.azure.com/role=agent -l storageprofile=managed --no-headers -o jsonpath="{.items[0].metadata.name}" >/dev/null 2>&1; then
