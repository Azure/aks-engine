...
DEPLOY_AND_INSTALL_AZUREDISKCSI="${DEPLOY_AND_INSTALL_AZUREDISKCSI:-false}"
...
function deployAndInstallAzurediskCSI {
  ...
  docker run --rm \
      -v $(pwd):${WORK_DIR} \
      -w ${WORK_DIR} \
      -e KUBECONFIG=_output/${RESOURCE_GROUP}/kubeconfig/kubeconfig.${LOCATION}.json \
      ${DEV_IMAGE} \
      /bin/bash -c "curl https://raw.githubusercontent.com/helm/helm/master/scripts/get-helm-3 | bash; helm repo add azuredisk-csi-driver https://raw.githubusercontent.com/kubernetes-sigs/azuredisk-csi-driver/master/charts; helm install azuredisk-csi-driver azuredisk-csi-driver/azuredisk-csi-driver --namespace kube-system --set cloud=AzureStackCloud --set controller.runOnMaster=true --version v1.10.0" || exit 1
}
...
if [ "${UPGRADE_CLUSTER}" = "true" ]; then
  if [ "${DEPLOY_AND_INSTALL_AZUREDISKCSI}" = "true" ]; then
    echo "Uninstalling azuredisk-csi-driver before upgrade..."
    docker run --rm \
      -v $(pwd):${WORK_DIR} \
      -w ${WORK_DIR} \
      -e KUBECONFIG=_output/${RESOURCE_GROUP}/kubeconfig/kubeconfig.${LOCATION}.json \
      ${DEV_IMAGE} \
      /bin/bash -c "helm uninstall azuredisk-csi-driver --namespace kube-system" || exit 1
  fi
  # modify the master VM SKU to simulate vertical vm scaling via upgrade
  docker run --rm \
      -v $(pwd):${WORK_DIR} \
  ...
  for ver_target in $UPGRADE_VERSIONS; do
    ...
    if [ "${DEPLOY_AND_INSTALL_AZUREDISKCSI}" = "true" ]; then
      echo "Installing azuredisk-csi-driver after upgrade..."
      docker run --rm \
        -v $(pwd):${WORK_DIR} \
        -w ${WORK_DIR} \
        -e KUBECONFIG=_output/${RESOURCE_GROUP}/kubeconfig/kubeconfig.${LOCATION}.json \
        ${DEV_IMAGE} \
        /bin/bash -c "helm repo add azuredisk-csi-driver https://raw.githubusercontent.com/kubernetes-sigs/azuredisk-csi-driver/master/charts; helm install azuredisk-csi-driver azuredisk-csi-driver/azuredisk-csi-driver --namespace kube-system --set cloud=AzureStackCloud --set controller.runOnMaster=true --version v1.10.0" || exit 1
    fi
