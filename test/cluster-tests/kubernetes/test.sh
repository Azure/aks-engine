...
function check_node_count() {
  ...
  if ((node_count != EXPECTED_NODE_COUNT)); then
    log "K8S: gave up waiting for apiserver / node counts"
    exit 1
  fi
}
...
if ((creating_count != 0)); then
  log "K8S: gave up waiting for containers"
  exit 1
fi
...
if [ -n "$(echo $pods | tr -d '[:space:]')" ]; then
  log "K8S: gave up waiting for running pods [$pods]"
  exit 1
fi
...
if ((running != EXPECTED_DNS)); then
  log "K8S: gave up waiting for kube-dns"
  exit 1
fi
...
if ((EXPECTED_DASHBOARD != 0)); then
  ...
  if ((running != EXPECTED_DASHBOARD)); then
    log "K8S: gave up waiting for kubernetes-dashboard"
    exit 1
  fi
else
...
if ((running != KUBE_PROXY_COUNT)); then
  log "K8S: gave up waiting for kube-proxy"
  exit 1
fi
...
if ! [ $EXPECTED_WINDOWS_AGENTS -gt 0 ]; then
  if ((EXPECTED_DASHBOARD != 0)); then
    ...
    for ip in $ips; do
      ...
      if [[ ${success} == "n" ]]; then
        log "K8S: gave up verifying proxy"
        exit 1
      fi
    done
  fi
